"""Tests for the deterministic L6.3 comparative assessment trace contract."""
...
def _result() -> ComparativeAssessmentResult:
    result = object.__new__(ComparativeAssessmentResult)
    object.__setattr__(result, "result_hash", HASH_A)
    object.__setattr__(result, "comparative_request_hash", HASH_B)
...
def _record(record_hash: str) -> ComparativeAssessmentRecord:
    ...
    object.__setattr__(record, "comparative_request_hash", HASH_B)
...
def test_trace_is_deterministic_and_content_addressed() -> None:
    ...
    first = build_comparative_assessment_trace(result, records=records)
    second = build_comparative_assessment_trace(result, records=records)

    assert first == second
    assert first.comparative_result_hash == HASH_A
    assert first.comparative_request_hash == HASH_B
    assert first.ordered_record_hashes == (HASH_C, HASH_D)
    assert first.comparative_trace_contract_version == "L6.3"
    assert first.trace_hash == sha256_payload(first.canonical_payload())


def test_trace_is_frozen_and_constructor_is_disabled() -> None:
    trace = build_comparative_assessment_trace(
        _result(),
        records=(_record(HASH_C), _record(HASH_D)),
    )
    trace_type: Any = ComparativeAssessmentTrace
    ...
    with pytest.raises(TypeError):
        trace_type(
            comparative_result_hash=HASH_A,
            comparative_request_hash=HASH_B,
            ordered_record_hashes=(HASH_C, HASH_D),
            comparative_trace_contract_version="L6.3",
            trace_hash=HASH_A,
        )
...
def test_wrong_lineage_order_and_duplicates_are_rejected() -> None:
    ...
    object.__setattr__(wrong, "comparative_request_hash", HASH_A)

    with pytest.raises(ValueError, match="result comparative request"):
        build_comparative_assessment_trace(
            result,
            records=(wrong, _record(HASH_D)),
        )

    with pytest.raises(ValueError, match="preserve comparative-result order"):
        build_comparative_assessment_trace(
            result,
...
def test_trace_module_excludes_prohibited_dependencies() -> None:
    module_path = Path("src/mh370_inverse_inference/comparative/trace.py")
