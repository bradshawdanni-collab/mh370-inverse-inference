...
def test_equal_magnitude_residuals_have_equal_likelihood() -> None:
    positive = GaussianEvidence(evidence_id="positive", residual=2.0, sigma=1.0)
    negative = GaussianEvidence(evidence_id="negative", residual=-2.0, sigma=1.0)

    assert gaussian_log_likelihood(positive) == pytest.approx(
        gaussian_log_likelihood(negative)
    )


def test_larger_sigma_penalizes_same_residual_less() -> None:
    tight = GaussianEvidence(evidence_id="tight", residual=4.0, sigma=1.0)
    wide = GaussianEvidence(evidence_id="wide", residual=4.0, sigma=2.0)

    assert gaussian_log_likelihood(wide) > gaussian_log_likelihood(tight)
...
def test_independent_log_likelihood_sums_terms() -> None:
    ...
    evidence = TrajectoryEvidence(trajectory_id="trajectory-a", terms=(first, second))
