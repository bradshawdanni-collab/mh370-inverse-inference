"""Bounded aircraft-state input and transition contracts for L1.1."""
...
class AircraftStateInput:
    """Immutable kinematic state copied from a governed observation."""
    ...
    @classmethod
    def from_radar_track_point(cls, point: RadarTrackPoint) -> AircraftStateInput:
        """Initialize a state input by copying one radar point exactly."""
    ...
    def to_payload(self) -> dict[str, Any]:
        """Return a canonical JSON-compatible state-input payload."""
        return {
...
class AircraftStateTransition:
    """Explicit input-state transition without a reachability claim."""

    previous: AircraftStateInput
    current: AircraftStateInput
    elapsed_seconds: float
    contract_version: str = AIRCRAFT_STATE_CONTRACT_VERSION

    def __post_init__(self) -> None:
        if type(self.previous) is not AircraftStateInput:
            raise TypeError("previous must be AircraftStateInput")
        if type(self.current) is not AircraftStateInput:
            raise TypeError("current must be AircraftStateInput")
